"""Lightweight regression tests for local Qwen node features.

Run from the ComfyUI-Qwen3 folder:
    ...
    def test_all_streaming_files_use_helper(self):
        for filename, expectations in STREAMING_FILES.items():
            ...
            with self.subTest(filename=filename):
                ...
                # For start_stage files, check the stage label
                if "stage" in expectations:
                    self.assertIn(expectations["stage"], source)
                # For compact files, just verify push_compact exists
                if expectations.get("compact"):
                    self.assertIn(".push_compact(", source)
...
class TestClassKeyCollision(unittest.TestCase):
    """Qwen3 class keys must NOT collide with ThinkingLLM ThinkingLLM_* keys."""

    def test_class_keys_stay_on_ailab(self):
        for filename in [
            "AILab_QwenVL.py",
            "AILab_QwenVL_GGUF.py",
            "AILab_QwenVL_PromptEnhancer.py",
            ...
            if mappings_block:
                ...
                with self.subTest(filename=filename):
                    self.assertTrue(
                        all(k.startswith("AILab_") for k in keys),
                        f"{filename} contains non-AILab_ class keys: {keys}",
                    )

    def test_no_thinkingllm_prefix_leak(self):
        for filename in [
            "AILab_QwenVL.py",
            "AILab_QwenVL_GGUF.py",
            "AILab_QwenVL_PromptEnhancer.py",
            ...
            source = read_source(filename)
            with self.subTest(filename=filename):
                self.assertNotIn("ThinkingLLM_", source)
